...
- Eight lesson days cover all 32 fights (packs: tactics, openings, endgames, defence) and deliberately mix packs, so a session cannot dead-end inside one pack. Days are picked on the home screen; `pack` survives only as a data label.
